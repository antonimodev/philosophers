					...
					int ac, char **av)
{
	if (!init_args(args, ac, av) ||
		...
		!init_philo_struct(params, args, philo)) // contiene philosophers_array que tiene malloc
		return (false);
	return (true);
}
...
bool    init_philo_struct(t_params *params, t_args *args, t_philo *philo)
{
	...
	if (!malloc_threads(params, args, philo) ||
		!malloc_philosophers_array(args, philo))
            return (false);
    i = 0;
